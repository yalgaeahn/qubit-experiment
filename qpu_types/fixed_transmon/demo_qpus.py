...
# 2025 TG JS SELECTIVE RIP
"""Fixed transmon qubit + 3D cavity RIP device setups for testing and demonstration."""
...
def demo_platform(n_qubits: int) -> QuantumPlatform:
    ...
    Returns:
        The QPU.
    """
    setup = fixed_transmon_setup(n_qubits)
    qubits = fixed_transmon_qubits(n_qubits, setup)
    quantum_operations = FixedTransmonOperations()
    qpu = QPU(qubits, quantum_operations=quantum_operations)
    return QuantumPlatform(setup=setup, qpu=qpu)


def fixed_transmon_setup(n_qubits: int) -> DeviceSetup:
    ...
    setup = DeviceSetup(f"fixed_transmons_{n_qubits}")
...
def fixed_transmon_qubits(
    n_qubits: int,
    setup: DeviceSetup,
) -> list[FixedTransmonQubit]:
    ...
    for i in range(n_qubits):
        q = FixedTransmonQubit.from_logical_signal_group(
            f"q{i}",
