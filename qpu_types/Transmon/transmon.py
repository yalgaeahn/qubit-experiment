...
@classformatter
@attrs.define()
class TransmonQubitParameters(QuantumParameters):
    """Qubit parameters for `TransmonQubit` instances.
    ...
    resonance_targ_frequency : float | None = None #임시방편
    ...
    #Cross-Resonance drive pulse parameters
...
class TransmonQubit(QuantumElement):
    ...
    TRANSITIONS = ("ge", "ef")
    ##########################################################
    def cr_parameters(self) -> tuple[str,dict]:
        """Return the cr drive parameters 
    ...
    def calibration(self) -> Calibration:  # noqa: C901, PLR0912
        ...
        ###############################################################
        if "drive_cr" in self.signals:
            sig_cal = SignalCalibration()
            if self.parameters.drive_frequency_cr is not None:
                sig_cal.oscillator = Oscillator(
                    uid=f"{self.uid}_drive_cr_osc",
                    frequency=self.parameters.drive_frequency_cr,
                    modulation_type=ModulationType.AUTO,
                )
            sig_cal.local_oscillator = drive_lo
            sig_cal.range = self.parameters.drive_range
            calibration_items[self.signals["drive_cr"]] = sig_cal    
        ###############################################################
