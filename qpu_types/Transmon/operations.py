...
from laboneq.dsl.experiment import pulse_library, builtins
...
class TransmonOperations(dsl.QuantumOperations):
    """Operations for TransmonQubits."""

    QUBIT_TYPES = TransmonQubit
    ...
    def barrier(self, q: TransmonQubit) -> None:
    ...
    @dsl.quantum_operation
    def z90(self, q: TransmonQubit, transition: str | None = None) -> None:
    ...
    @dsl.quantum_operation
    def z180(self, q: TransmonQubit, transition: str | None = None) -> None:
    ...
    def direct_cr(
        ...
        with dsl.section(
            name=f"cr_{ctrl.uid}c_{targ.uid}t",
            on_system_grid=True, # align to 8ns grid 
            alignment = SectionAlignment.LEFT
        ):
            drive_line, params = ctrl.cr_parameters()
...
def x180_ef_reset_pulse(
    ...
    **_,
    ...
    time = 0.5 * (x + 1) * length
    wfm = pulse_library.pulse_factory(pulse_func)(**pls_kwags).evaluate(x)
