...
from qubit_experiment.experiments.two_qubit_readout_calibration import (
...
def resolve_convergence_repeats_per_state(repeats_per_state: int) -> int:
    ...
    return repeats
# =====================================================================
# 1) MODIFIED experiment_workflow
# =====================================================================
...
def experiment_workflow(
    ...
    """Run 2Q tomography with optional RIP preparation and spectator qubit.

    Args:
        spec:
            Optional spectator qubit. Prepared at the start of each tomography
            setting but not measured. If None, no spectator preparation is done.
        spec_prep:
            Spectator preparation: "nop"/"g" → ground, "pi"/"e" → excited.
        targ_detuning:
            Virtual detuning (Hz) for the target Bell-closing Y90 pulse.
    """
    options = (
    ...
    temp_spec = None
    if spec is not None:
        temp_spec = temporary_quantum_elements_from_qpu(temp_qpu, spec)
...
def _resolve_bus_rip_length(bus_elem) -> float:
    """Resolve RIP pulse length from bus parameters."""
    length = getattr(bus_elem.parameters, "rip_length", None)
    if length is None:
        return 0.0
    return float(length)


def _resolve_spec_state(spec_prep: str) -> str:
    """Map spec_prep label to prepare_state argument."""
    s = spec_prep.strip().lower()
    if s in ("nop", "no_pi", "0", "g", "ground"):
        return "g"
    if s in ("pi", "x", "1", "e", "excited"):
        return "e"
    raise ValueError(
        f"Unsupported spec_prep {spec_prep!r}. Use 'nop'/'g' or 'pi'/'e'."
    )

# =====================================================================
# 3) MODIFIED create_experiment
# =====================================================================
...
def create_experiment(
    ...
    """Create 2Q tomography experiment with optional RIP + spectator.

    Pulse sequence per tomography setting::

        [spec_prep] → ctrl_prep → targ_prep
        → RIP(buses)
        → targ: Y90(φ = 2π × detuning × rip_length)  [bell close]
        → phase_reset(-φ)
        → basis_rotation → measure(ctrl, targ)
    """
    ...
    # ✅ NEW: spectator validation
    spec_elem = None
    spec_state = "g"
    if spec is not None:
        spec_elem = validation.validate_and_convert_single_qubit_sweeps(spec)
        spec_state = _resolve_spec_state(spec_prep)
    ...
    # Compute Bell-closing phase
    targ_bell_phase = 0.0
    if use_rip and abs(targ_detuning) > 0:
        rip_length = _resolve_bus_rip_length(buses[0])
        targ_bell_phase = (2 * np.pi * targ_detuning * rip_length) % (2 * np.pi)
        ...
        for setting_label, (ctrl_axis, targ_axis) in TOMOGRAPHY_SETTINGS:
            with dsl.section(
                ...
                if spec_elem is not None:
                    spec_kwargs = {
                        "name": f"spec_prep_{setting_label}",
                        "alignment": SectionAlignment.LEFT,
                    }
                    if prep_play_after is not None:
                        spec_kwargs["play_after"] = prep_play_after
                    with dsl.section(**spec_kwargs) as spec_sec:
                        qop.prepare_state(spec_elem, spec_state)
                    prep_play_after = spec_sec.uid
                ...
                if use_rip:
                    ...
                    # ── Bell-closing Y90 on targ ────────────────────
                    if abs(targ_bell_phase) > 1e-12:
                        with dsl.section(
                            name=f"bell_close_{setting_label}",
                            alignment=SectionAlignment.LEFT,
                            play_after=rip_sec.uid,
                        ) as bell_close_sec:
                            qop.y90(
                                targ,
                                increment_oscillator_phase=targ_bell_phase,
                            )

                        with dsl.section(
                            name=f"phase_reset_{setting_label}",
                            alignment=SectionAlignment.LEFT,
                            play_after=bell_close_sec.uid,
                        ) as phase_reset_sec:
                            dsl.play(
                                signal=targ.signals["drive"],
                                pulse=None,
                                increment_oscillator_phase=-targ_bell_phase,
                            )
                        basis_play_after = phase_reset_sec.uid  # ✅ FIX: was bell_close_sec
