...
@workflow.workflow(name="rip_zzz_interaction")
def experiment_workflow(
    ...
    spec_prep: str = "nop",         # ✅ 추가 ("pi" or "nop")
    c_prep: str = "g",
    ...
    spec = temporary_quantum_elements_from_qpu(temp_qpu, spec)   # ✅ 추가
...
def create_experiment(
    ...
    spec = validation.validate_and_convert_qubits_sweeps(spec)   # ✅ 추가
    ...
    # "pi"/"nop"를 prepare_state 입력("e"/"g")으로 매핑
    ...
    # spec π pulse on/off sweep: 0 = no pi pulse, 1 = pi pulse
    swp_spec = SweepParameter(
        uid="spec_pi_sweep",
        values=np.array([0, 1]),     # 0: id(no pulse), 1: x180
    )
    ...
    with dsl.acquire_loop_rt(
        ...
        with dsl.sweep(
            ...
            with dsl.section(name="main", alignment=SectionAlignment.LEFT):
                ...
                # ✅ 1) spec prep 섹션
                with dsl.section(name="spec_prep", alignment=SectionAlignment.LEFT) as sec_spec_prep:
                    for q_s in spec:
                ...
                with dsl.section(
                    ...
                    for q_t, q_c, q_b, q_b2, q_b3, wait_time, phase in zip(
                        ...
                        # Target Ramsey
                        # qop.ramsey(q_t, wait_time, phase, transition=opts.transition)
                        qop.ramsey.omit_section(q_t, wait_time, phase, transition=opts.transition)
