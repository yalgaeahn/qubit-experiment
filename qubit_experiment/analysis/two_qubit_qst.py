"""Two-qubit QST analysis with MLE reconstruction and convergence summaries.

This module provides the single-run 2Q QST analysis used by
`experiments.two_qubit_qst`, plus notebook-derived product-state convergence and
shot-sweep summaries. The single-run path supports only INTEGRATION-based,
single-shot tomography data and reuses readout calibration shots for IQ-domain
discrimination and assignment-matrix mitigation.
"""
...
class TwoQQstAnalysisOptions:
    """Options for single-run 2Q QST analysis."""
...
@workflow.workflow(name="analysis_two_qubit_qst")
def analysis_workflow(
    ...
    options = TwoQQstAnalysisOptions() if options is None else options
...
def _fit_discriminator_from_readout_calibration_impl(
    ...
    if readout_calibration_result is None:
        raise ValueError(
            "readout_calibration_result is required for INTEGRATION-based QST analysis."
        )
...
def fit_discriminator_from_readout_calibration(
    ...
    return _fit_discriminator_from_readout_calibration_impl(
        readout_calibration_result=readout_calibration_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
    )
...
def _collect_tomography_counts_impl(
    tomography_result: RunExperimentResults,
    ctrl_uid: str,
    targ_uid: str,
...
def collect_tomography_counts(
    ...
    return _collect_tomography_counts_impl(
        tomography_result=tomography_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
...
def _extract_assignment_matrix_impl(
    readout_calibration_result: RunExperimentResults | None,
    ctrl_uid: str,
    targ_uid: str,
    discriminator: dict[str, object],
    ...
    if readout_calibration_result is None:
        raise ValueError(
            "readout_calibration_result is required for assignment extraction."
        )
...
def extract_assignment_matrix(
    ...
    """Extract 4x4 assignment matrix from IQ posterior probabilities."""
    return _extract_assignment_matrix_impl(
        readout_calibration_result=readout_calibration_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
        discriminator=discriminator,
...
def _maximum_likelihood_reconstruct_impl(
...
def maximum_likelihood_reconstruct(
    ...
    """Reconstruct the density matrix with readout-mitigated MLE."""
    return _maximum_likelihood_reconstruct_impl(
...
def _calculate_state_metrics_impl(
...
def calculate_state_metrics(
    ...
    return _calculate_state_metrics_impl(
        rho_hat_real=rho_hat_real,
        rho_hat_imag=rho_hat_imag,
        target_state=target_state,
    )


def _evaluate_optimization_convergence_impl(
    tomography_counts: dict[str, list],
...
def evaluate_optimization_convergence(
    ...
    return _evaluate_optimization_convergence_impl(
        tomography_counts=tomography_counts,
...
def _build_analysis_payload(
    ...
    discriminator = _fit_discriminator_from_readout_calibration_impl(
        readout_calibration_result=readout_calibration_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
    )
    assignment = _extract_assignment_matrix_impl(
        readout_calibration_result=readout_calibration_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
        discriminator=discriminator,
    )
    tomography_counts = _collect_tomography_counts_impl(
        tomography_result=tomography_result,
        ctrl_uid=ctrl_uid,
        targ_uid=targ_uid,
        discriminator=discriminator,
    )
    mle_result = _maximum_likelihood_reconstruct_impl(
        tomography_counts=tomography_counts,
        assignment=assignment,
        max_iterations=max_iterations,
    )
    state_metrics = _calculate_state_metrics_impl(
        rho_hat_real=mle_result["rho_hat_real"],
        rho_hat_imag=mle_result["rho_hat_imag"],
        target_state=target_state,
    )
    optimization_convergence = _evaluate_optimization_convergence_impl(
        tomography_counts=tomography_counts,
...
def _assemble_analysis_output_from_tasks(analysis_node):
    ...
    analysis = _task_output(tasks, "analyze_tomography_run")
    ...
    if not all(
        isinstance(x, dict)
        for x in [assignment, tomography_counts, mle, state_metrics, discriminator]
    ):
        return None
...
def _find_analysis_node(root, depth=0, max_depth=12):
    ...
    if tasks:
        ...
        if "analyze_tomography_run" in names or (
...
def collect_convergence_run_record(
    ...
    """Extract compact per-run convergence record from an analysis output."""
...
def collect_shot_sweep_run_record(
    ...
    """Analyze one shot-sweep run and return a row plus optional failure metadata."""
    failure = None
    try:
...
def _validate_shot_sweep_run_records_impl(
    ...
    for row in run_records or []:
        ...
        infid = _to_float_or_nan(row.get("infidelity"))
        if np.isfinite(infid) and (infid < (float(eps) - 1e-15) or infid > (1.0 + float(infid_tol))):
...
def _aggregate_shot_sweep_statistics_impl(
    ...
    for row in run_records or []:
        if not isinstance(row, dict):
            continue
        grouped[(str(row.get("state", "")), int(row.get("log2_shots", -1)))].append(row)

    aggregated_rows: list[dict[str, object]] = []
    for (state, log2_shots), items in sorted(grouped.items(), key=lambda x: (x[0][0], x[0][1])):
        if not items:
            continue
...
def aggregate_shot_sweep_statistics(
    ...
    """Aggregate shot-sweep rows into notebook-style per-state/per-shot summaries."""
...
def _summarize_final_shot_sweep_impl(
    ...
    if not shot_log2_values:
        return []
    last_log2 = max(int(v) for v in shot_log2_values)
    rows = []
...
def summarize_final_shot_sweep(
    ...
    """Summarize the largest-shot operating point from aggregated shot-sweep stats."""
...
def plot_convergence_suite_fidelity(
    ...
    if states:
        means = np.array(
            [
                per_state[s].get("fidelity_mean")
                if per_state[s].get("fidelity_mean") is not None
                else np.nan
                for s in states
            ],
            dtype=float,
        ...
        errs = np.array(
            [
                per_state[s].get("fidelity_ci95")
                if per_state[s].get("fidelity_ci95") is not None
                else np.nan
                for s in states
            ],
            dtype=float,
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    workflow.save_artifact("twoq_qst_convergence_fidelity", fig)
...
def plot_shot_sweep_summary(
    ...
    for state in suite_states:
        state_rows = sorted(grouped.get(str(state), []), key=lambda r: int(r.get("log2_shots", -1)))
        if not state_rows:
            continue
        x = np.asarray([row.get("log2_shots", np.nan) for row in state_rows], dtype=float)

        y_infid = np.asarray([row.get("infid_mean", np.nan) for row in state_rows], dtype=float)
        e_infid = np.asarray([row.get("infid_ci95", np.nan) for row in state_rows], dtype=float)
        ...
        y_log = np.asarray([row.get("log10_infid_mean", np.nan) for row in state_rows], dtype=float)
        e_log = np.asarray([row.get("log10_infid_ci95", np.nan) for row in state_rows], dtype=float)
    ...
    fig.suptitle("2Q QST Product-State Suite Shot Sweep", y=1.02, fontsize=14)
    fig.tight_layout()
    workflow.save_artifact("twoq_qst_shot_sweep_summary", fig)
...
def plot_density_matrix(
    ...
    """Plot real and imaginary parts of the reconstructed 2Q density matrix."""
    ...
    fig.tight_layout()
    workflow.save_artifact("twoq_qst_density_matrix", fig)
...
def plot_counts(
    ...
    fig.tight_layout()
    workflow.save_artifact("twoq_qst_counts", fig)
    return {"counts": fig}
