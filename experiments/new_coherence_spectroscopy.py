...
def create_experiment(
    ...
    ramsey_section_length = swp_delays + 2 * transition_params["length_pi2"]
    ...
    opts.ring_up
        + (np.array(delays, dtype=float) + 2 * transition_params["length_pi2"])
        ...
        with dsl.sweep(
            ...
            with dsl.sweep(
                ...
                with dsl.section(name="qubit_drive", alignment=SectionAlignment.LEFT):
                    ...
                    with dsl.section(name="main_measure", alignment=SectionAlignment.LEFT, play_after=ramsey_sec.uid) as readout_sec:
                        qop.measure(q, dsl.handles.result_handle(q.uid))
                        # Fix the length of the measure section
                        readout_sec.length = readout_section_length
            qop.passive_reset(q)
